...
- **模型路由**：默认用主模型（不随会话变）；⌘+点击选过模型并开启开关后，固定用所选模型（优先级：显式选择 > 主模型）。注意：「固定模型」需要宿主支持 llm.oneshot 的 provider/model 参数（已提 Hermes 官方 PR，合并发布前的 Hermes 版本开启后会提示版本不兼容报错，关闭开关即用主模型正常增强）
