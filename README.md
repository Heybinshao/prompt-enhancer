# Prompt Enhancer for Hermes ![版本](https://img.shields.io/badge/版本-v1.4.0-blue)
...
**想用固定的模型做增强（v1.4.0 新增）**：⌘+点击 ✨ 按钮弹出模型选择器——开关打开并挑一个模型，之后增强固定用它，不随会话变；开关关闭则回落主模型。选择会被记住，重启后仍生效。
...
- **模型路由**：默认用主模型（不随会话变）；⌘+点击选过模型并开启开关后，固定用所选模型（优先级：显式选择 > 主模型；Hermes 旧版本宿主会忽略插件传的模型参数，自动回落继承行为）
- **多行布局**：草稿换行后输入框进入堆叠布局（官方行为）；按钮位于控件行内，多行态下可能靠行首（官方控件行布局问题，已提 hermes-agent#116332 待修复）
- **发送后**：按钮自动回到空闲态（增强结果已随消息发出）

## 卸载

「技能与工具 → 插件」里找到 Prompt Enhancer → 关闭开关；或直接删除 `~/.hermes/desktop-plugins/prompt-enhancer/` 文件夹后重启 Hermes。
...
- **增强结果保留官方渲染**：草稿里已有的 `@引用` 和 `/命令` pill 在增强后原样保留（图标、配色与官方一致）；模型新引入的命令词以纯文本发送，消息气泡里仍会正常渲染
- 草稿超过 8000 字符暂不支持
...
- **固定增强用模型（v1.4.0 起插件内直接选）**：⌘+点击 ✨ 选模型即可，无需改配置。也可以在 Hermes 的 `config.yaml` 配（两种方式效果一致，配置优先级更低）：
...
按钮注入官方 `composer.actions` 贡献区；读取草稿（复刻官方 `composerPlainText` 序列化语义）→ 调用 `llm.oneshot` 网关 RPC（无状态、不污染 prompt cache；默认裸请求落主模型，⌘+点击选过模型则附带 provider/model 参数显式钉路，旧版宿主静默忽略并回落继承行为）→ DOM 写回 + 官方 input 事件镜像进草稿 store。写回前做快照比对，草稿在等待期被手改则放弃应用——绝不覆盖用户的字。
