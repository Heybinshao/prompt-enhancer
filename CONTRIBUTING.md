...
tests/               # 回归用例（读磁盘入口做断言，入口路径见硬红线 4）
...
4. **测试/脚本里引用入口一律写 `desktop/plugin.js`**，不要写 `../plugin.js`。改入口路径后必须重跑 `node tests/regression.test.mjs`（当前 10/10）。
...
3. 回归用例全绿：`node tests/regression.test.mjs`
4. 四处版本号对齐：**git tag** · **仓库 About** · **README 版本徽章** · **`plugin.yaml` 的 `version`**
5. tag → release（正文写清「新增 / 修复 / 变更」）→ push

> 官方插件目录里的版本**不会**自动跟上这个仓库。目录条目钉的是某个已评审的 commit——不提交 pin 更新 PR，目录用户就停在那一版。这是设计如此，不是故障。

## 提 issue / PR

欢迎提 issue 报 bug 或说需求：附上 `~/.hermes/logs/desktop.log` 里 `[prompt-enhancer]` 开头的行即可，不用截图也不用复现步骤。PR 请先开 issue 说清动机；改动请严格遵守上面的结构约束。
